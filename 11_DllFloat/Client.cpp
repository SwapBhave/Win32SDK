...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		hLib = LoadLibrary("Server.dll");
		pfnArea = (FuncPtr)GetProcAddress(hLib, "Area");
		Result =pfnArea(Radius);
		sprintf_s(str,TEXT("%f"),Result);
		MessageBoxA(hwnd, str, TEXT("Output"), MB_OK);
		FreeLibrary(hLib);
		break;
	case WM_DESTROY:
		PostQuitMessage(0);
